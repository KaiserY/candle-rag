...
impl LlamaCppModel {
  pub fn load_model(&mut self) -> anyhow::Result<()> {
    ...
    let mut model_file = std::fs::File::open(&self.model_path)?;
    let start = std::time::Instant::now();
    let device = candle_examples::device(true)?;

    self.model_weights = match model_path.extension().and_then(|v| v.to_str()) {
      Some("gguf") => {
        let model =
      ...
      Some("ggml" | "bin") | Some(_) | None => {
        ...
        let default_gqa = match self.model_id {
    ...
    let tokenizer =
      Tokenizer::from_file(PathBuf::from(&self.tokenizer_path)).map_err(anyhow::Error::msg)?;

    self.token_output_stream = TokenOutputStream::new(tokenizer);

    Ok(())
  }

  pub fn run_model(&mut self, setting: &ModelChatSetting) -> anyhow::Result<()> {
    ...
    for prompt_index in 0.. {
      ...
      print!("{}", &prompt_str);
      let tokens = self.token_output_stream
        .tokenizer()
      ...
      let eos_token = if self.model_id.is_open_chat() {
        "<|end_of_turn|>"
      } else {
        "</s>"
      };
      let eos_token = *self.token_output_stream.tokenizer().get_vocab(true).get(eos_token).unwrap();
      ...
      if let Some(rest) = self.token_output_stream.decode_rest().map_err(candle_core::Error::msg)? {
        print!("{rest}");
      }
      std::io::stdout().flush()?;
